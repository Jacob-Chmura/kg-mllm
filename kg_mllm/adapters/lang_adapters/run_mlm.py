...
class ModelArguments:
    """Arguments to which model/config/tokenizer we fine-tune, or train from scratch."""
    ...
    config_overrides: Optional[str] = field(
        ...
        metadata={
            'help':
            'Override default config when a model is trained from scratch.'
        },
    )
    ...
    use_fast_tokenizer: bool = field(
        ...
        metadata={
            'help':
            'Whether to use one of the fast tokenizer (tokenizers library).'
        },
    )
    model_revision: str = field(
        ...
        metadata={
            'help':
            'Specific model version to use (branch name, tag name, commit id).'
        },
    )
    use_auth_token: bool = field(
        ...
        metadata={
            'help':
            'Use the token generated when running `huggingface-cli login`'
        },
    )
...
class DataTrainingArguments:
    """Arguments pertaining to what input to our model for training and eval."""
    ...
    validation_file: Optional[str] = field(
        ...
        metadata={
            'help':
            'An optional input data file to evaluate perplexity on (a text file).'
        },
    )
    ...
    validation_split_percentage: Optional[int] = field(
        ...
        metadata={
            'help':
            'Percentage of train set used as validation when no validation split'
        },
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        # If no validation data, validation_split_percentage used to divide the dataset.
        if 'validation' not in raw_datasets.keys():
            raw_datasets['validation'] = load_dataset(
                extension,
                data_files=data_files,
    ...
    if model_args.tokenizer_name:
        ...
        raise ValueError(
            ...
            'You can do it from another script, save it, and load it '
    ...
    # a smaller embedding size, remove this test.
    ...
    if data_args.max_seq_length is None:
        ...
        if max_seq_length > 1024:
            logger.warning(
                ...
                'by passing --max_seq_length xxx.')
            max_seq_length = 1024
    else:
        if data_args.max_seq_length > tokenizer.model_max_length:
            logger.warning(
                f'The max_seq_length passed ({data_args.max_seq_length}) is larger '
                f'than the max length for the model ({tokenizer.model_max_length}). '
    ...
    pad_to_multiple_of_8 = (data_args.line_by_line and training_args.fp16
